...
class _HomeDriverState extends State<HomeDriver> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Escuchar el radar para mostrar el banner
  ...
  Future<bool> _handlePermissions() async {
    ...
    if (permission == LocationPermission.denied) {
      ...
      if (permission == LocationPermission.denied) {
        _showErrorSnackBar('Permiso de ubicación denegado. No puedes ponerte online.');
        return false;
      }
    }
    
    if (permission == LocationPermission.deniedForever) {
      _showErrorSnackBar('El permiso de ubicación está bloqueado permanentemente.');
      return false;
    }
    return true;
  }
  ...
  void _startBroadcasting() {
    // Iniciar el Radar con la primera posición
    ...
    // Iniciar el Stream de geolocalización real
        ...
        children: [
          ...
          SafeArea(
            child: Padding(
              ...
              child: _buildStateToggle(),
            ),
          ),

          // Panel Inferior Reactivo
          Align(
  ...
  Widget _buildMapContent() {
    // Si hay un viaje en curso, mostrar MapTracker
    ...
    return Container(
      ...
      child: const Center(child: Icon(Icons.map, color: Colors.white24, size: 60)),
    );
  }
      ...
      child: Row(
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              color: _isOnline ? const Color(0xFFFF6B00) : Colors.white60,
  ...
  Widget _buildBottomUI() {
    // Escuchar el estado global para determinar qué panel mostrar
    return StreamBuilder<Map<String, Trip>>(
      ...
      builder: (context, snapshot) {
        ...
        final inProgressTrip = trips.values.where((t) => 
          t.driverId == widget.driverId && 
          (t.status == TripStatus.accepted || t.status == TripStatus.inProgress)
        ).firstOrNull;

        if (inProgressTrip != null) {
          return _buildInProgressSection(inProgressTrip);
        }

        if (_isOnline) {
          return _buildRadarSection();
        }
    ...
    return StreamBuilder<List<Trip>>(
      ...
      builder: (context, snapshot) {
        ...
        if (nearbyTrips.isEmpty) {
          return _buildInfoPanel("Buscando viajes cercanos...");
        }
        ...
        return Container(
          ...
          child: PageView.builder(
            ...
            itemBuilder: (context, index) => _buildTripCard(nearbyTrips[index]),
          ),
        );
      },
    );
  ...
  Widget _buildTripCard(Trip trip) {
    final distance = _currentPosition != null && trip.route.isNotEmpty
        ? (GeoUtils.calculateDistance(_currentPosition!, trip.route.first) / 1000).toStringAsFixed(1)
        : "?";
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text("Nuevo Viaje - $distance km", style: const TextStyle(color: Color(0xFFFF6B00), fontWeight: FontWeight.bold)),
              Text("\$25.0", style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
            ],
          ),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              onPressed: () => TripAcceptProtocol.acceptTrip(trip: trip, driverId: widget.driverId, etaInMinutes: 5),
  ...
  Widget _buildInProgressSection(Trip trip) {
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(trip.status == TripStatus.accepted ? "YENDO POR PASAJERO" : "VIAJE EN CURSO", 
               style: const TextStyle(color: Color(0xFFFF6B00), fontWeight: FontWeight.bold)),
          const SizedBox(height: 16),
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: trip.status == TripStatus.accepted ? const Color(0xFFFF6B00) : Colors.green,
            ...
            onPressed: () {
              if (trip.status == TripStatus.accepted) {
                Antigravity.mutateTrip(
                  currentTrip: trip,
                  nextTrip: trip.copyWith(status: TripStatus.inProgress),
                  onCommit: (t) => Antigravity.emit('trip.in_progress', {'tripId': t.id}),
                  onRollback: (_) => _showErrorSnackBar("Error al iniciar viaje"),
                );
              } else {
                Antigravity.mutateTrip(
                  currentTrip: trip,
                  nextTrip: trip.copyWith(status: TripStatus.completed),
                  onCommit: (t) => Antigravity.emit('trip.completed', {'tripId': t.id}),
                  onRollback: (_) => _showErrorSnackBar("Error al finalizar viaje"),
                );
              }
            },
        ...
        child: Container(
          ...
          child: const Row(
            children: [
              ...
              SizedBox(width: 12),
              Text("¡NUEVO VIAJE DISPONIBLE!", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            ],
          ),
        ),
      ),
